...
class SOMVisualizer:
    def __init__(self, som, data, target, target_names):
        self.som = som
        self.data = data
        self.target = target
        self.target_names = target_names

    def plot(self, grid_type='square', label_type='cluster'):
        ...
        if grid_type == 'square':
            if label_type == 'cluster':
                self.plot_som_cluster_labels()
            elif label_type == 'block':
                self.plot_som_block_labels()
        elif grid_type == 'hexagonal':
            if label_type == 'cluster':
                self.plot_hexagonal_som_cluster_labels()
            elif label_type == 'block':
                self.plot_hexagonal_som_block_labels()
    ...
    def plot_som_cluster_labels(self):
        ...
        for label, center in cluster_centers.items():
            plt.text(center[1], center[0], self.target_names[label], ha='center', va='center', color='white', fontsize=12, fontweight='bold')
    ...
    def plot_som_block_labels(self):
        plt.figure(figsize=(10, 10))
        ...
        for x in range(self.som.x_size):
            for y in range(self.som.y_size):
                plt.text(y, x, self.target_names[node_labels[x, y]], ha='center', va='center', color='white', fontsize=10)
    ...
    def plot_hexagonal_som_cluster_labels(self):
        plt.figure(figsize=(10, 10))
        ...
        for label, center in cluster_centers.items():
            ...
            plt.text(hex_center[0], hex_center[1], self.target_names[label], ha='center', va='center', color='white', fontsize=12, fontweight='bold')
    ...
    def plot_hexagonal_som_block_labels(self):
        plt.figure(figsize=(10, 10))
        ...
        for x in range(self.som.x_size):
            for y in range(self.som.y_size):
                ...
                hex_patch = Polygon(hex_coords, facecolor=plt.cm.viridis(hex_color), edgecolor='k', lw=1)
                plt.gca().add_patch(hex_patch)
                plt.text(hex_center[0], hex_center[1], self.target_names[node_labels[x, y]], ha='center', va='center', color='white', fontsize=10)
